...
// 完全免费的语音播放器
class FreeVoicePlayer {
    constructor() {
        this.strategies = [
            this.googleTTS.bind(this),
            this.localTTS.bind(this)
        ];
...
// 创建全局语音播放器实例
const freeVoicePlayer = new FreeVoicePlayer();
...
function initSpeechSynthesis() {
    // 现在使用免费的语音播放器，不需要复杂的初始化
    console.log('使用免费语音播放器，无需复杂初始化');
}
...
function speakCurrentWord() {
    ...
    // 使用免费语音播放器
    freeVoicePlayer.speak(word.word).then(success => {
...
function showWordShowcasePage() {
    ...
    wordCardsContainer.querySelectorAll('.showcase-speak-btn').forEach(btn => {
        btn.addEventListener('click', function(e) {
            ...
            // 使用免费语音播放器
            freeVoicePlayer.speak(wordObj.word);
        });
    });
}
